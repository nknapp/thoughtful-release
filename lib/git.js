...
var cpp = require('../lib/promised-child-process')
...
function Git (dir) {
  ...
  this.isRepo = function isRepo () {
    return fs.stat(path.join(dir, '.git')).then((stat) => {
  ...
  this.squashRebaseTodos = function squashRebaseTodos (todoFile) {
    return fs.readFile(todoFile, 'utf-8')
      .then((todo) => {
        var counter = 0
      ...
      .then((todo) => fs.writeFile(todoFile, todo))
  }
    ...
    return this.run('tag', '-f', `thoughtful-backup/cleanup-history-${timestamp}`) // Make a backup...
      .then((tagOutput) => {
        return this.run('merge-base', 'HEAD', targetBranch) // Get merge-base
          ...
          .then(() => this.run('rebase', targetBranch))
          .then((execFileResult) => execFileResult.appendTo(tagOutput.appendTo('')))
      })
  }
