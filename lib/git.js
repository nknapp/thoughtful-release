...
var cpp = require('../lib/cp-promised')
...
function Git(dir, options) {
  ...
  this.lastRelease = function lastReleaseTag() {
    return cpp.execFile('git', ['describe', '--long', '--match', 'v*']).then((output) => {
   ...
   * @param {string=} to the end tag. This defaults to the current commit if not set.
   */
  this.changelog = function changelog(from, options) {
    ...
    return cpp.execFile('git', ['log', '--no-merges', `--pretty=tformat:* ${commitPattern} %s - %an`, `${from}..${to}` ])
  }
}
